...
public class SpeaktoitRecognitionServiceImpl extends AIService implements VoiceActivityDetector.SpeechEventsListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener {
    ...
    private void startRecording(RequestExtras requestExtras) {
        ...
        onListeningStarted();
        this.recognizeTask = new RecognizeTask(new RecorderStream(this.audioRecord), requestExtras);
    ...
    private class RecorderStream extends InputStream {
        ...
        public int read(@NonNull byte[] bArr, int i, int i2) throws IOException {
            ...
            if (read > 0) {
                synchronized (this.bytesLock) {
                    ...
                    this.bytes = new byte[(length + read)];
                    if (length > 0) {
                        System.arraycopy(bArr2, 0, this.bytes, 0, length);
                    }
                    System.arraycopy(bArr, 0, this.bytes, length, read);
                    while (this.bytes.length >= 320) {
                        byte[] bArr3 = new byte[VoiceActivityDetector.FRAME_SIZE_IN_BYTES];
                        System.arraycopy(this.bytes, 0, bArr3, 0, VoiceActivityDetector.FRAME_SIZE_IN_BYTES);
                        SpeaktoitRecognitionServiceImpl.this.vad.processBuffer(bArr3, VoiceActivityDetector.FRAME_SIZE_IN_BYTES);
                        byte[] bArr4 = this.bytes;
                        int length2 = bArr4.length - VoiceActivityDetector.FRAME_SIZE_IN_BYTES;
                        this.bytes = new byte[length2];
                        System.arraycopy(bArr4, VoiceActivityDetector.FRAME_SIZE_IN_BYTES, this.bytes, 0, length2);
                    }
        ...
        private void normalize(@NonNull byte[] bArr, int i) {
            ...
            if (i >= i2) {
                ...
                for (int i3 = 0; i3 < asShortBuffer.limit(); i3++) {
                    ...
                    this.alignment = (((this.count - 1.0f) / this.count) * this.alignment) + (((float) s) / this.count);
                    this.count += 1.0f;
                }
                this.extent = Math.max(Math.abs(this.max), Math.abs(this.min));
                float f = (this.dbLevel * 32767.0f) / ((float) this.extent);
                for (int i4 = 0; i4 < asShortBuffer.limit(); i4++) {
                    order.putShort((short) ((int) ((((float) asShortBuffer.get(i4)) - this.alignment) * f)));
                }
            }
