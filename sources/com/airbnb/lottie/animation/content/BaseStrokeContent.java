...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C0687L.beginSection("StrokeContent#draw");
        if (C0744Utils.hasZeroScaleAxis(matrix)) {
            C0687L.endSection("StrokeContent#draw");
            return;
        }
        ...
        this.paint.setStrokeWidth(((FloatKeyframeAnimation) this.widthAnimation).getFloatValue() * C0744Utils.getScale(matrix));
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C0687L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C0687L.endSection("StrokeContent#buildPath");
                C0687L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C0687L.endSection("StrokeContent#drawPath");
            }
        }
        C0687L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        C0687L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C0687L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            if (floatValue3 > length) {
                ...
                if (f3 < f + length2 && f < f3) {
                    C0744Utils.applyTrimPathIfNeeded(this.trimPathPath, floatValue2 > length ? (floatValue2 - length) / length2 : 0.0f, Math.min(f3 / length2, 1.0f), 0.0f);
            ...
            if (f4 >= floatValue2 && f <= floatValue3) {
                if (f4 > floatValue3 || floatValue2 >= f) {
                    ...
                    C0744Utils.applyTrimPathIfNeeded(this.trimPathPath, f5, f2, 0.0f);
                    canvas.drawPath(this.trimPathPath, this.paint);
                } else {
                    canvas.drawPath(this.trimPathPath, this.paint);
                }
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        C0687L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        float floatValue = ((FloatKeyframeAnimation) this.widthAnimation).getFloatValue() / 2.0f;
        ...
        C0687L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        C0687L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C0687L.endSection("StrokeContent#applyDashPattern");
            return;
        }
        float scale = C0744Utils.getScale(matrix);
        ...
        this.paint.setPathEffect(new DashPathEffect(this.dashPatternValues, this.dashPatternOffsetAnimation == null ? 0.0f : scale * this.dashPatternOffsetAnimation.getValue().floatValue()));
        C0687L.endSection("StrokeContent#applyDashPattern");
    }
