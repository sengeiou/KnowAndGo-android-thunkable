...
public final class C0744Utils {
    ...
    public static void applyTrimPathIfNeeded(Path path, float f, float f2, float f3) {
        C0687L.beginSection("applyTrimPathIfNeeded");
        pathMeasure.setPath(path, false);
        float length = pathMeasure.getLength();
        if (f == 1.0f && f2 == 0.0f) {
            C0687L.endSection("applyTrimPathIfNeeded");
    ...
    public static void saveLayerCompat(Canvas canvas, RectF rectF, Paint paint, int i) {
        C0687L.beginSection("Utils#saveLayer");
        if (Build.VERSION.SDK_INT < 23) {
            canvas.saveLayer(rectF, paint, i);
        } else {
            canvas.saveLayer(rectF, paint);
        }
        C0687L.endSection("Utils#saveLayer");
    }
