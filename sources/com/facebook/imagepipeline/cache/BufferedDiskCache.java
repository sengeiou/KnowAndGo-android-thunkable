...
public class BufferedDiskCache {
    ...
    private Task<Boolean> containsAsync(final CacheKey cacheKey) {
        try {
            ...
            FLog.m101w(TAG, (Throwable) e, "Failed to schedule disk-cache read for %s", cacheKey.getUriString());
    ...
    public boolean checkInStagingAreaAndFileCache(CacheKey cacheKey) {
        ...
        if (encodedImage != null) {
            encodedImage.close();
            FLog.m83v(TAG, "Found image for %s in staging area", (Object) cacheKey.getUriString());
            this.mImageCacheStatsTracker.onStagingAreaHit(cacheKey);
            return true;
        }
        FLog.m83v(TAG, "Did not find image for %s in staging area", (Object) cacheKey.getUriString());
                ...
                public EncodedImage call() throws Exception {
                    ...
                    try {
                        if (!atomicBoolean.get()) {
                            ...
                            if (encodedImage != null) {
                                FLog.m83v((Class<?>) BufferedDiskCache.TAG, "Found image for %s in staging area", (Object) cacheKey.getUriString());
                                BufferedDiskCache.this.mImageCacheStatsTracker.onStagingAreaHit(cacheKey);
                            } else {
                                FLog.m83v((Class<?>) BufferedDiskCache.TAG, "Did not find image for %s in staging area", (Object) cacheKey.getUriString());
                                ...
                                try {
                                    ...
                                    of = CloseableReference.m126of(access$400);
                            ...
                            FLog.m82v((Class<?>) BufferedDiskCache.TAG, "Host thread was interrupted, decreasing reference count");
                            if (encodedImage != null) {
                                encodedImage.close();
                            }
    ...
    public void put(final CacheKey cacheKey, EncodedImage encodedImage) {
        ...
        try {
            ...
            FLog.m101w(TAG, (Throwable) e, "Failed to schedule disk-cache write for %s", cacheKey.getUriString());
    ...
    public Task<Void> remove(final CacheKey cacheKey) {
        ...
        try {
            ...
            FLog.m101w(TAG, (Throwable) e, "Failed to schedule disk-cache remove for %s", cacheKey.getUriString());
            return Task.forError(e);
        }
    }
    ...
    public Task<Void> clearAll() {
        ...
        try {
            ...
            FLog.m101w(TAG, (Throwable) e, "Failed to schedule disk-cache clear", new Object[0]);
            return Task.forError(e);
        }
    }
    ...
    public PooledByteBuffer readFromDiskCache(CacheKey cacheKey) throws IOException {
        ...
        try {
            FLog.m83v(TAG, "Disk cache read for %s", (Object) cacheKey.getUriString());
            BinaryResource resource = this.mFileCache.getResource(cacheKey);
            if (resource == null) {
                FLog.m83v(TAG, "Disk cache miss for %s", (Object) cacheKey.getUriString());
                this.mImageCacheStatsTracker.onDiskCacheMiss(cacheKey);
                return null;
            }
            FLog.m83v(TAG, "Found entry in disk cache for %s", (Object) cacheKey.getUriString());
            ...
            openStream.close();
            FLog.m83v(TAG, "Successful read from disk cache for %s", (Object) cacheKey.getUriString());
            return newByteBuffer;
        } catch (IOException e) {
            FLog.m101w(TAG, (Throwable) e, "Exception reading from cache for %s", cacheKey.getUriString());
    ...
    public void writeToDiskCache(CacheKey cacheKey, final EncodedImage encodedImage) {
        FLog.m83v(TAG, "About to write to disk-cache for key %s", (Object) cacheKey.getUriString());
        try {
            ...
            FLog.m83v(TAG, "Successful disk-cache write for key %s", (Object) cacheKey.getUriString());
        } catch (IOException e) {
            FLog.m101w(TAG, (Throwable) e, "Failed to write to disk-cache for key %s", cacheKey.getUriString());
        }
    }
}
