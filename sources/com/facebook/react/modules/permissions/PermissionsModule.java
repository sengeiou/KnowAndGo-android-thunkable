...
public class PermissionsModule extends ReactContextBaseJavaModule implements PermissionListener {
    ...
    public void requestPermission(final String str, final Promise promise) {
        Context baseContext = getReactApplicationContext().getBaseContext();
        if (Build.VERSION.SDK_INT < 23) {
            promise.resolve(baseContext.checkPermission(str, Process.myPid(), Process.myUid()) == 0 ? PermissionsResponse.GRANTED_KEY : "denied");
    ...
    public void requestMultiplePermissions(ReadableArray readableArray, final Promise promise) {
        ...
        for (int i2 = 0; i2 < readableArray.size(); i2++) {
            ...
            if (Build.VERSION.SDK_INT < 23) {
                writableNativeMap.putString(string, baseContext.checkPermission(string, Process.myPid(), Process.myUid()) == 0 ? PermissionsResponse.GRANTED_KEY : "denied");
                i++;
