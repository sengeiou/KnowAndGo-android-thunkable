...
public class CatalystInstanceImpl implements CatalystInstance {
    ...
    public void callFunction(PendingJSCall pendingJSCall) {
        if (this.mDestroyed) {
            ...
            FLog.m102w(ReactConstants.TAG, "Calling JS function after bridge has been destroyed: " + pendingJSCall2);
            return;
        }
    ...
    public void invokeCallback(int i, NativeArrayInterface nativeArrayInterface) {
        if (this.mDestroyed) {
            FLog.m102w(ReactConstants.TAG, "Invoking JS callback after bridge has been destroyed.");
        } else {
            ...
            while (true) {
                if (!this.mNativeModulesThreadDestructionComplete || !this.mJSThreadDestructionComplete) {
                    if (System.currentTimeMillis() - currentTimeMillis > 100) {
                        Log.w(ReactConstants.TAG, "CatalystInstanceImpl.destroy() timed out waiting for Native Modules and JS thread teardown");
                        break;
                    }
                } else {
                    break;
