...
public abstract class BaseViewManager<T extends View, C extends LayoutShadowNode> extends ViewManager<T, C> implements BaseViewManagerInterface<T> {
    ...
    static {
        sStateDescription.put(STATE_BUSY, Integer.valueOf(C1129R.string.state_busy_description));
        sStateDescription.put(STATE_EXPANDED, Integer.valueOf(C1129R.string.state_expanded_description));
        sStateDescription.put("collapsed", Integer.valueOf(C1129R.string.state_collapsed_description));
    }
    ...
    public void setAccessibilityHint(@NonNull T t, @Nullable String str) {
        t.setTag(C1129R.C1132id.accessibility_hint, str);
        updateViewContentDescription(t);
    }
    ...
    public void setAccessibilityRole(@NonNull T t, @Nullable String str) {
        if (str != null) {
            t.setTag(C1129R.C1132id.accessibility_role, ReactAccessibilityDelegate.AccessibilityRole.fromValue(str));
        }
    }
    ...
    public void setViewState(@NonNull T t, @Nullable ReadableMap readableMap) {
        if (readableMap != null) {
            t.setTag(C1129R.C1132id.accessibility_state, readableMap);
            t.setSelected(false);
            t.setEnabled(true);
    ...
    private void updateViewContentDescription(@NonNull T t) {
        Dynamic dynamic;
        String str = (String) t.getTag(C1129R.C1132id.accessibility_label);
        ReadableMap readableMap = (ReadableMap) t.getTag(C1129R.C1132id.accessibility_state);
        String str2 = (String) t.getTag(C1129R.C1132id.accessibility_hint);
        ArrayList arrayList = new ArrayList();
        ReadableMap readableMap2 = (ReadableMap) t.getTag(C1129R.C1132id.accessibility_value);
        if (str != null) {
            arrayList.add(str);
        }
        if (readableMap != null) {
            ...
            while (keySetIterator.hasNextKey()) {
                ...
                if (nextKey.equals(STATE_CHECKED) && dynamic2.getType() == ReadableType.String && dynamic2.asString().equals(STATE_MIXED)) {
                    arrayList.add(t.getContext().getString(C1129R.string.state_mixed_description));
                } else if (nextKey.equals(STATE_BUSY) && dynamic2.getType() == ReadableType.Boolean && dynamic2.asBoolean()) {
                    arrayList.add(t.getContext().getString(C1129R.string.state_busy_description));
                } else if (nextKey.equals(STATE_EXPANDED) && dynamic2.getType() == ReadableType.Boolean) {
                    arrayList.add(t.getContext().getString(dynamic2.asBoolean() ? C1129R.string.state_expanded_description : C1129R.string.state_collapsed_description));
                }
            }
        }
    ...
    public void setAccessibilityActions(T t, ReadableArray readableArray) {
        if (readableArray != null) {
            t.setTag(C1129R.C1132id.accessibility_actions, readableArray);
        }
    }
    ...
    public void setAccessibilityValue(T t, ReadableMap readableMap) {
        if (readableMap != null) {
            t.setTag(C1129R.C1132id.accessibility_value, readableMap);
    ...
    public Map<String, Object> getExportedCustomDirectEventTypeConstants() {
        return MapBuilder.builder().put("topAccessibilityAction", MapBuilder.m138of("registrationName", "onAccessibilityAction")).build();
    }
    ...
    private void logUnsupportedPropertyWarning(String str) {
        FLog.m104w(ReactConstants.TAG, "%s doesn't support property '%s'", getName(), str);
    }
}
