...
public class SsManifestParser implements ParsingLoadable.Parser<SsManifest> {
    ...
    private static class StreamIndexParser extends ElementParser {
        ...
        private void parseStreamFragmentStartTag(XmlPullParser xmlPullParser) throws ParserException {
            ...
            long parseLong = parseLong(xmlPullParser, KEY_FRAGMENT_START_TIME, C1470C.TIME_UNSET);
            int i = 1;
            if (parseLong == C1470C.TIME_UNSET) {
                if (size == 0) {
                    parseLong = 0;
            ...
            this.lastChunkDuration = parseLong(xmlPullParser, KEY_FRAGMENT_DURATION, C1470C.TIME_UNSET);
            long parseLong2 = parseLong(xmlPullParser, KEY_FRAGMENT_REPEAT_COUNT, 1);
            if (parseLong2 <= 1 || this.lastChunkDuration != C1470C.TIME_UNSET) {
                while (true) {
                    long j = (long) i;
                    if (j < parseLong2) {
