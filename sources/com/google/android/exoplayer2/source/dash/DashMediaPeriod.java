...
final class DashMediaPeriod implements MediaPeriod, SequenceableLoader.Callback<ChunkSampleStream<DashChunkSource>>, ChunkSampleStream.ReleaseCallback<DashChunkSource> {
    ...
    public void updateManifest(DashManifest dashManifest, int i) {
        ...
        for (EventSampleStream eventSampleStream : this.eventSampleStreams) {
            ...
            while (true) {
                ...
                if (next.mo19689id().equals(eventSampleStream.eventStreamId())) {
                    boolean z = true;
    ...
    private static int[][] getGroupedAdaptationSetIndices(List<AdaptationSet> list) {
        ...
        for (int i = 0; i < size; i++) {
            sparseIntArray.put(list.get(i).f155id, i);
        }
    ...
    private static int buildPrimaryAndEmbeddedTrackGroupInfos(List<AdaptationSet> list, int[][] iArr, int i, boolean[] zArr, boolean[] zArr2, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr) {
        ...
        while (i6 < i5) {
            ...
            if (i3 != -1) {
                trackGroupArr[i3] = new TrackGroup(Format.createSampleFormat(adaptationSet.f155id + ":emsg", MimeTypes.APPLICATION_EMSG, (String) null, -1, (DrmInitData) null));
                trackGroupInfoArr[i3] = TrackGroupInfo.embeddedEmsgTrack(iArr2, i7);
            }
            if (i2 != -1) {
                trackGroupArr[i2] = new TrackGroup(Format.createTextSampleFormat(adaptationSet.f155id + ":cea608", MimeTypes.APPLICATION_CEA608, 0, (String) null));
    ...
    private static void buildManifestEventTrackGroupInfos(List<EventStream> list, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr, int i) {
        ...
        int i3 = 0;
        while (i3 < list.size()) {
            trackGroupArr[i2] = new TrackGroup(Format.createSampleFormat(list.get(i3).mo19689id(), MimeTypes.APPLICATION_EMSG, (String) null, -1, (DrmInitData) null));
