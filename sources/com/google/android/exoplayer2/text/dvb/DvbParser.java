...
final class DvbParser {
    ...
    private static void parseSubtitlingSegment(ParsableBitArray parsableBitArray, SubtitleService subtitleService2) {
        ...
        if (readBits3 * 8 > parsableBitArray.bitsLeft()) {
            Log.m157w(TAG, "Data field length exceeds limit");
            ...
            case 17:
                ...
                if (readBits2 == subtitleService2.subtitlePageId && pageComposition2 != null) {
                    ...
                    if (pageComposition2.state == 0) {
                        parseRegionComposition.mergeFrom(subtitleService2.regions.get(parseRegionComposition.f161id));
                    }
                    subtitleService2.regions.put(parseRegionComposition.f161id, parseRegionComposition);
                    break;
                }
            case 18:
                if (readBits2 != subtitleService2.subtitlePageId) {
                    if (readBits2 == subtitleService2.ancillaryPageId) {
                        ...
                        subtitleService2.ancillaryCluts.put(parseClutDefinition.f159id, parseClutDefinition);
                        break;
                    }
                } else {
                    ClutDefinition parseClutDefinition2 = parseClutDefinition(parsableBitArray, readBits3);
                    subtitleService2.cluts.put(parseClutDefinition2.f159id, parseClutDefinition2);
                    break;
                }
                break;
            case 19:
                if (readBits2 != subtitleService2.subtitlePageId) {
                    if (readBits2 == subtitleService2.ancillaryPageId) {
                        ...
                        subtitleService2.ancillaryObjects.put(parseObjectData.f160id, parseObjectData);
                        break;
                    }
                } else {
                    ObjectData parseObjectData2 = parseObjectData(parsableBitArray);
                    subtitleService2.objects.put(parseObjectData2.f160id, parseObjectData2);
                    break;
                }
                break;
            case 20:
