...
final class ExoPlayerImpl extends BasePlayer implements ExoPlayer {
    ...
    public ExoPlayerImpl(Renderer[] rendererArr, TrackSelector trackSelector2, LoadControl loadControl, BandwidthMeter bandwidthMeter, Clock clock, Looper looper) {
        ...
        Log.m155i(TAG, "Init " + Integer.toHexString(System.identityHashCode(this)) + " [" + ExoPlayerLibraryInfo.VERSION_SLASHY + "] [" + Util.DEVICE_DEBUG_INFO + "]");
    ...
    public void seekTo(int i, long j) {
        ...
        if (timeline.isEmpty()) {
            this.maskingWindowPositionMs = j == C1470C.TIME_UNSET ? 0 : j;
            this.maskingPeriodIndex = 0;
        } else {
            long defaultPositionUs = j == C1470C.TIME_UNSET ? timeline.getWindow(i, this.window).getDefaultPositionUs() : C1470C.msToUs(j);
            Pair<Object, Long> periodPosition = timeline.getPeriodPosition(this.window, this.period, i, defaultPositionUs);
            this.maskingWindowPositionMs = C1470C.usToMs(defaultPositionUs);
            this.maskingPeriodIndex = timeline.getIndexOfPeriod(periodPosition.first);
        }
        this.internalPlayer.seekTo(timeline, i, C1470C.msToUs(j));
        Iterator<Player.EventListener> it = this.listeners.iterator();
        while (it.hasNext()) {
    ...
    public void release() {
        Log.m155i(TAG, "Release " + Integer.toHexString(System.identityHashCode(this)) + " [" + ExoPlayerLibraryInfo.VERSION_SLASHY + "] [" + Util.DEVICE_DEBUG_INFO + "] [" + ExoPlayerLibraryInfo.registeredModules() + "]");
        this.mediaSource = null;
    ...
    public long getDuration() {
        ...
        return C1470C.usToMs(this.period.getAdDurationUs(mediaPeriodId.adGroupIndex, mediaPeriodId.adIndexInAdGroup));
    }
    ...
    public long getCurrentPosition() {
        ...
        if (this.playbackInfo.periodId.isAd()) {
            return C1470C.usToMs(this.playbackInfo.positionUs);
        }
    ...
    public long getBufferedPosition() {
        ...
        if (this.playbackInfo.loadingMediaPeriodId.equals(this.playbackInfo.periodId)) {
            return C1470C.usToMs(this.playbackInfo.bufferedPositionUs);
        }
        return getDuration();
    }

    public long getTotalBufferedDuration() {
        return Math.max(0, C1470C.usToMs(this.playbackInfo.totalBufferedDurationUs));
    }
    ...
    public long getContentPosition() {
        ...
        return this.period.getPositionInWindowMs() + C1470C.usToMs(this.playbackInfo.contentPositionUs);
    }
    ...
    private void handlePlaybackInfo(PlaybackInfo playbackInfo2, int i, boolean z, int i2) {
        ...
        if (this.pendingOperationAcks == 0) {
            if (playbackInfo2.startPositionUs == C1470C.TIME_UNSET) {
