...
abstract class zzdju implements zzdlk {
    ...
    public final byte[] zzo(byte[] bArr) throws GeneralSecurityException {
        if (bArr.length <= Integer.MAX_VALUE - zzauv()) {
            ByteBuffer allocate = ByteBuffer.allocate(zzauv() + bArr.length);
            zza(allocate, bArr);
    ...
    public final void zza(ByteBuffer byteBuffer, byte[] bArr) throws GeneralSecurityException {
        if (byteBuffer.remaining() - zzauv() >= bArr.length) {
            byte[] zzff = zzdlo.zzff(zzauv());
            byteBuffer.put(zzff);
            ByteBuffer wrap = ByteBuffer.wrap(bArr);
            int remaining = wrap.remaining();
            int i = (remaining / 64) + 1;
            for (int i2 = 0; i2 < i; i2++) {
                ByteBuffer zzc = zzc(zzff, this.zzgyg + i2);
                if (i2 == i - 1) {
                    zzdjs.zza(byteBuffer, wrap, zzc, remaining % 64);
                } else {
                    zzdjs.zza(byteBuffer, wrap, zzc, 64);
                }
            }
            return;
        }
    ...
    static void zza(int[] iArr, int[] iArr2) {
        System.arraycopy(zzgye, 0, iArr, 0, zzgye.length);
