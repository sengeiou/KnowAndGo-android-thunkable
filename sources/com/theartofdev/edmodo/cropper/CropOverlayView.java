...
public class CropOverlayView extends View {
    ...
    private void initCropWindow() {
        ...
        if (min > max && min2 > max2) {
            ...
            float f = min - max;
            float f2 = this.mInitialCropWindowPaddingRatio * f;
            float f3 = min2 - max2;
            float f4 = this.mInitialCropWindowPaddingRatio * f3;
    ...
    private boolean calculateBounds(RectF rectF) {
        ...
        float f = this.mBoundsPoints[0];
        float f2 = this.mBoundsPoints[1];
        float f3 = this.mBoundsPoints[4];
        float f4 = this.mBoundsPoints[5];
        float f5 = this.mBoundsPoints[6];
        float f6 = this.mBoundsPoints[7];
        if (this.mBoundsPoints[7] < this.mBoundsPoints[1]) {
            if (this.mBoundsPoints[1] < this.mBoundsPoints[3]) {
                f = this.mBoundsPoints[6];
                f2 = this.mBoundsPoints[7];
                f3 = this.mBoundsPoints[2];
                f4 = this.mBoundsPoints[3];
                f5 = this.mBoundsPoints[4];
                f6 = this.mBoundsPoints[5];
            } else {
                f = this.mBoundsPoints[4];
                f2 = this.mBoundsPoints[5];
                f3 = this.mBoundsPoints[0];
                f4 = this.mBoundsPoints[1];
                f5 = this.mBoundsPoints[2];
                f6 = this.mBoundsPoints[3];
            }
        } else if (this.mBoundsPoints[1] > this.mBoundsPoints[3]) {
            f = this.mBoundsPoints[2];
            f2 = this.mBoundsPoints[3];
            f3 = this.mBoundsPoints[6];
            f4 = this.mBoundsPoints[7];
            f5 = this.mBoundsPoints[0];
            f6 = this.mBoundsPoints[1];
        }
        float f7 = (f6 - f2) / (f5 - f);
        float f8 = -1.0f / f7;
        float f9 = f2 - (f7 * f);
        ...
        float f18 = rectBottom;
        if (f17 >= rectF2.right) {
            f17 = rectLeft;
        }
        float max = Math.max(rectLeft, f17);
        float f19 = (f14 - f10) / (f8 - centerY);
        if (f19 >= rectF2.right) {
            f19 = max;
        }
        float max2 = Math.max(max, f19);
        float f20 = f8 - f13;
        float f21 = (f15 - f12) / f20;
        if (f21 >= rectF2.right) {
            f21 = max2;
        }
        float max3 = Math.max(max2, f21);
        float f22 = (f15 - f10) / f20;
        if (f22 <= rectF2.left) {
            f22 = rectRight;
        }
        float min = Math.min(rectRight, f22);
        float f23 = (f15 - f11) / (f7 - f13);
        if (f23 <= rectF2.left) {
            f23 = min;
        }
        float min2 = Math.min(min, f23);
        float f24 = (f14 - f11) / f16;
        if (f24 <= rectF2.left) {
            f24 = min2;
        }
        float min3 = Math.min(min2, f24);
        float max4 = Math.max(rectTop, Math.max((f7 * max3) + f9, (f8 * min3) + f10));
        float min4 = Math.min(f18, Math.min((f8 * max3) + f12, (f7 * min3) + f11));
