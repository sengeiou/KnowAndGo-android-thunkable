...
public class PanGestureHandler extends GestureHandler<PanGestureHandler> {
    ...
    private boolean shouldActivate() {
        ...
        if (this.mMinVelocityX != MIN_VALUE_IGNORE && ((this.mMinVelocityX < 0.0f && f4 <= this.mMinVelocityX) || (this.mMinVelocityX >= 0.0f && f4 >= this.mMinVelocityX))) {
            return true;
        }
        float f5 = this.mLastVelocityY;
        if (this.mMinVelocityY != MIN_VALUE_IGNORE && ((this.mMinVelocityY < 0.0f && f4 <= this.mMinVelocityY) || (this.mMinVelocityY >= 0.0f && f4 >= this.mMinVelocityY))) {
            return true;
        }
        float f6 = (f4 * f4) + (f5 * f5);
        if (this.mMinVelocitySq == MIN_VALUE_IGNORE || f6 < this.mMinVelocitySq) {
            return false;
        }
        return true;
    }
    ...
    private boolean shouldFail() {
        ...
        float f2 = (this.mLastY - this.mStartY) + this.mOffsetY;
        if (this.mFailOffsetYStart != MAX_VALUE_IGNORE && f2 < this.mFailOffsetYStart) {
            return true;
        }
        if (this.mFailOffsetYEnd == MIN_VALUE_IGNORE || f2 <= this.mFailOffsetYEnd) {
            return false;
        }
        return true;
    }
