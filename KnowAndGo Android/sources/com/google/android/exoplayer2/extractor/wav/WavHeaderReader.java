...
final class WavHeaderReader {
    ...
    public static WavHeader peek(ExtractorInput extractorInput) throws IOException, InterruptedException {
        ...
        if (ChunkHeader.peek(extractorInput, parsableByteArray).f139id != WavUtil.RIFF_FOURCC) {
            return null;
        }
        ...
        if (readInt != WavUtil.WAVE_FOURCC) {
            Log.m153e(TAG, "Unsupported RIFF format: " + readInt);
            return null;
        }
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.f139id != WavUtil.FMT_FOURCC) {
        ...
        if (readLittleEndianUnsignedShort3 == i) {
            ...
            if (encodingForType == 0) {
                Log.m153e(TAG, "Unsupported WAV format: " + readLittleEndianUnsignedShort4 + " bit/sample, type " + readLittleEndianUnsignedShort);
                return null;
            }
    ...
    public static void skipToData(ExtractorInput extractorInput, WavHeader wavHeader) throws IOException, InterruptedException {
        ...
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.f139id != Util.getIntegerCodeForString("data")) {
            Log.m157w(TAG, "Ignoring unknown WAV chunk: " + peek.f139id);
            long j = peek.size + 8;
            if (peek.f139id == Util.getIntegerCodeForString("RIFF")) {
                j = 12;
            }
            if (j <= 2147483647L) {
                ...
                throw new ParserException("Chunk is too large (~2GB+) to skip; id: " + peek.f139id);
            }
        }
