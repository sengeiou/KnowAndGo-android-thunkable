...
public final class SilenceSkippingAudioProcessor implements AudioProcessor {
    ...
    private void processMaybeSilence(ByteBuffer byteBuffer) {
        ...
        int length = this.maybeSilenceBuffer.length - this.maybeSilenceBufferSize;
        if (findNoisePosition >= limit || position >= length) {
    ...
    private int findNoisePosition(ByteBuffer byteBuffer) {
        for (int position = byteBuffer.position() + 1; position < byteBuffer.limit(); position += 2) {
            if (Math.abs(byteBuffer.get(position)) > 4) {
                return this.bytesPerFrame * (position / this.bytesPerFrame);
            }
        }
    ...
    private int findNoiseLimit(ByteBuffer byteBuffer) {
        for (int limit = byteBuffer.limit() - 1; limit >= byteBuffer.position(); limit -= 2) {
            if (Math.abs(byteBuffer.get(limit)) > 4) {
                return (this.bytesPerFrame * (limit / this.bytesPerFrame)) + this.bytesPerFrame;
            }
        }
