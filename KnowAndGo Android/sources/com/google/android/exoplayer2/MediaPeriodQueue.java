...
final class MediaPeriodQueue {
    ...
    public boolean shouldLoadNextMediaPeriod() {
        return this.loading == null || (!this.loading.info.isFinal && this.loading.isFullyBuffered() && this.loading.info.durationUs != C1470C.TIME_UNSET && this.length < 100);
    }
    ...
    public MediaPeriodHolder advancePlayingPeriod() {
        if (this.playing != null) {
            ...
            if (this.length == 0) {
                ...
                this.oldFrontPeriodWindowSequenceNumber = this.playing.info.f134id.windowSequenceNumber;
            }
    ...
    public void clear(boolean z) {
        ...
        if (frontPeriod != null) {
            ...
            this.oldFrontPeriodWindowSequenceNumber = frontPeriod.info.f134id.windowSequenceNumber;
            frontPeriod.release();
    ...
    public MediaPeriodInfo getUpdatedMediaPeriodInfo(MediaPeriodInfo mediaPeriodInfo) {
        long durationUs;
        boolean isLastInPeriod = isLastInPeriod(mediaPeriodInfo.f134id);
        boolean isLastInTimeline = isLastInTimeline(mediaPeriodInfo.f134id, isLastInPeriod);
        this.timeline.getPeriodByUid(mediaPeriodInfo.f134id.periodUid, this.period);
        if (mediaPeriodInfo.f134id.isAd()) {
            durationUs = this.period.getAdDurationUs(mediaPeriodInfo.f134id.adGroupIndex, mediaPeriodInfo.f134id.adIndexInAdGroup);
        } else {
            durationUs = mediaPeriodInfo.f134id.endPositionUs == Long.MIN_VALUE ? this.period.getDurationUs() : mediaPeriodInfo.f134id.endPositionUs;
        }
        return new MediaPeriodInfo(mediaPeriodInfo.f134id, mediaPeriodInfo.startPositionUs, mediaPeriodInfo.contentPositionUs, durationUs, isLastInPeriod, isLastInTimeline);
    }
    ...
    private long resolvePeriodIndexToWindowSequenceNumber(Object obj) {
        ...
        for (MediaPeriodHolder frontPeriod = getFrontPeriod(); frontPeriod != null; frontPeriod = frontPeriod.next) {
            if (frontPeriod.uid.equals(obj)) {
                return frontPeriod.info.f134id.windowSequenceNumber;
            }
        }
        for (MediaPeriodHolder frontPeriod2 = getFrontPeriod(); frontPeriod2 != null; frontPeriod2 = frontPeriod2.next) {
            ...
            if (indexOfPeriod2 != -1 && this.timeline.getPeriod(indexOfPeriod2, this.period).windowIndex == i) {
                return frontPeriod2.info.f134id.windowSequenceNumber;
            }
        }
    ...
    private boolean canKeepMediaPeriodHolder(MediaPeriodHolder mediaPeriodHolder, MediaPeriodInfo mediaPeriodInfo) {
        ...
        return mediaPeriodInfo2.startPositionUs == mediaPeriodInfo.startPositionUs && mediaPeriodInfo2.f134id.equals(mediaPeriodInfo.f134id);
    }
    ...
    private MediaPeriodInfo getFollowingMediaPeriodInfo(MediaPeriodHolder mediaPeriodHolder, long j) {
        ...
        if (mediaPeriodInfo.isLastInTimelinePeriod) {
            int nextPeriodIndex = this.timeline.getNextPeriodIndex(this.timeline.getIndexOfPeriod(mediaPeriodInfo.f134id.periodUid), this.period, this.window, this.repeatMode, this.shuffleModeEnabled);
            if (nextPeriodIndex == -1) {
                return null;
            }
            int i = this.timeline.getPeriod(nextPeriodIndex, this.period, true).windowIndex;
            Object obj2 = this.period.uid;
            long j6 = mediaPeriodInfo.f134id.windowSequenceNumber;
            if (this.timeline.getWindow(i, this.window).firstPeriodIndex == nextPeriodIndex) {
                Pair<Object, Long> periodPosition = this.timeline.getPeriodPosition(this.window, this.period, i, C1470C.TIME_UNSET, Math.max(0, rendererOffset));
                ...
                if (mediaPeriodHolder2.next == null || !mediaPeriodHolder2.next.uid.equals(obj3)) {
                    ...
                    j4 = mediaPeriodHolder2.next.info.f134id.windowSequenceNumber;
                }
                j5 = longValue;
                j3 = j4;
                obj = obj3;
        ...
        MediaSource.MediaPeriodId mediaPeriodId = mediaPeriodInfo.f134id;
        ...
        if (mediaPeriodId.isAd()) {
            ...
            if (nextAdIndexToPlay >= adCountInAdGroup) {
                ...
                if (this.period.getAdGroupCount() == 1 && this.period.getAdGroupTimeUs(0) == 0) {
                    Pair<Object, Long> periodPosition2 = this.timeline.getPeriodPosition(this.window, this.period, this.period.windowIndex, C1470C.TIME_UNSET, Math.max(0, rendererOffset));
        ...
        } else if (mediaPeriodInfo.f134id.endPositionUs != Long.MIN_VALUE) {
            int adGroupIndexForPositionUs = this.period.getAdGroupIndexForPositionUs(mediaPeriodInfo.f134id.endPositionUs);
            if (adGroupIndexForPositionUs == -1) {
                return getMediaPeriodInfoForContent(mediaPeriodId.periodUid, mediaPeriodInfo.f134id.endPositionUs, mediaPeriodId.windowSequenceNumber);
            }
            ...
            return getMediaPeriodInfoForAd(mediaPeriodId.periodUid, adGroupIndexForPositionUs, firstAdIndexToPlay, mediaPeriodInfo.f134id.endPositionUs, mediaPeriodId.windowSequenceNumber);
        } else {
    ...
    private MediaPeriodInfo getMediaPeriodInfoForContent(Object obj, long j, long j2) {
        ...
        this.timeline.getPeriodByUid(mediaPeriodId.periodUid, this.period);
        boolean isLastInPeriod = isLastInPeriod(mediaPeriodId);
        return new MediaPeriodInfo(mediaPeriodId, j, C1470C.TIME_UNSET, j3 == Long.MIN_VALUE ? this.period.getDurationUs() : j3, isLastInPeriod, isLastInTimeline(mediaPeriodId, isLastInPeriod));
    }
