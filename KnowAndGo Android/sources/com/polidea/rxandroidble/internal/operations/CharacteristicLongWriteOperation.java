...
public class CharacteristicLongWriteOperation extends QueueOperation<byte[]> {
    ...
    public void protectedRun(Emitter<byte[]> emitter, QueueReleaseInterface queueReleaseInterface) throws Throwable {
        ...
        if (payloadSizeLimit > 0) {
            ...
            writeBatchAndObserve(payloadSizeLimit, wrap).subscribeOn(this.bluetoothInteractionScheduler).takeFirst(writeResponseForMatchingCharacteristic(this.bluetoothGattCharacteristic)).timeout(this.timeoutConfiguration.timeout, this.timeoutConfiguration.timeoutTimeUnit, error, this.timeoutConfiguration.timeoutScheduler).repeatWhen(m1094x3cbe3949(this.writeOperationAckStrategy, wrap, queueReleasingEmitterWrapper)).retryWhen(errorIsRetryableAndAccordingTo(this.writeOperationRetryStrategy, wrap, payloadSizeLimit)).toCompletable().subscribe(new Action0() {
                public void call() {
    ...
    private static Func1<Observable<? extends Void>, Observable<?>> m1094x3cbe3949(final RxBleConnection.WriteOperationAckStrategy writeOperationAckStrategy2, final ByteBuffer byteBuffer, final QueueReleasingEmitterWrapper<byte[]> queueReleasingEmitterWrapper) {
                ...
                return new Func1<Throwable, Observable<RxBleConnection.WriteOperationRetryStrategy.LongWriteFailure>>() {
                    public Observable<RxBleConnection.WriteOperationRetryStrategy.LongWriteFailure> call(Throwable th) {
                        if ((th instanceof BleGattCharacteristicException) || (th instanceof BleGattCannotStartException)) {
                            return Observable.just(new RxBleConnection.WriteOperationRetryStrategy.LongWriteFailure(C26976.this.calculateFailedBatchIndex(byteBuffer, i), (BleGattException) th));
                        }
