...
public class PartialDiskCacheProducer implements Producer<EncodedImage> {
        ...
        return new Continuation<EncodedImage, Void>() {
            public Void then(Task<EncodedImage> task) throws Exception {
                if (PartialDiskCacheProducer.isTaskCancelled(task)) {
                    ...
                    if (result != null) {
                        producerListener.onProducerFinishWithSuccess(producerContext2, PartialDiskCacheProducer.PRODUCER_NAME, PartialDiskCacheProducer.getExtraMap(producerListener, producerContext2, true, result.getSize()));
                        ...
                        producerListener.onProducerFinishWithSuccess(producerContext2, PartialDiskCacheProducer.PRODUCER_NAME, PartialDiskCacheProducer.getExtraMap(producerListener, producerContext2, false, 0));
    ...
    static Map<String, String> getExtraMap(ProducerListener2 producerListener2, ProducerContext producerContext, boolean z, int i) {
        ...
        if (z) {
            return ImmutableMap.m35of("cached_value_found", String.valueOf(z), "encodedImageSize", String.valueOf(i));
        }
        return ImmutableMap.m34of("cached_value_found", String.valueOf(z));
    }
    ...
    private static class PartialDiskCacheConsumer extends DelegatingConsumer<EncodedImage, EncodedImage> {
            ...
            if (!isNotLast(i)) {
                if (this.mPartialEncodedImageFromCache != null && encodedImage.getBytesRange() != null) {
                    try {
                        ...
                        FLog.m63e(PartialDiskCacheProducer.PRODUCER_NAME, "Error while merging image data", (Throwable) e);
        ...
        private void sendFinalResultToConsumer(PooledByteBufferOutputStream pooledByteBufferOutputStream) {
            ...
            Throwable th;
            CloseableReference of = CloseableReference.m126of(pooledByteBufferOutputStream.toByteBuffer());
            try {
