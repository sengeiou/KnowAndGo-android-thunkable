    ...
    private static void buildSpannedFromShadowNode(ReactBaseTextShadowNode reactBaseTextShadowNode, SpannableStringBuilder spannableStringBuilder, List<SetSpanOperation> list, TextAttributes textAttributes, boolean z, Map<Integer, ReactShadowNode> map, int i) {
        ...
        for (int i3 = 0; i3 < childCount; i3++) {
            ...
            if (childAt instanceof ReactRawTextShadowNode) {
                ...
                list2.add(new SetSpanOperation(spannableStringBuilder.length() - INLINE_VIEW_PLACEHOLDER.length(), spannableStringBuilder.length(), ((ReactTextInlineImageShadowNode) childAt).buildInlineImageSpan()));
            } else if (z) {
                ...
                if (styleWidth.unit == YogaUnit.POINT && styleHeight.unit == YogaUnit.POINT) {
                    ...
                    spannableStringBuilder2.append(INLINE_VIEW_PLACEHOLDER);
                    list2.add(new SetSpanOperation(spannableStringBuilder.length() - INLINE_VIEW_PLACEHOLDER.length(), spannableStringBuilder.length(), new TextInlineViewPlaceholderSpan(reactTag, (int) f, (int) f2)));
