...
public class PermissionsActivity extends Activity {
    ...
    private void requestPermission() {
        if (Build.VERSION.SDK_INT < 23) {
            finish();
            overridePendingTransition(C2463R.anim.onesignal_fade_in, C2463R.anim.onesignal_fade_out);
        } else if (!waiting) {
            waiting = true;
    ...
    private void showLocationPermissionSettings() {
        new AlertDialog.Builder(ActivityLifecycleHandler.curActivity).setTitle(C2463R.string.location_not_available_title).setMessage(C2463R.string.location_not_available_open_settings_message).setPositiveButton(C2463R.string.location_not_available_open_settings_option, new DialogInterface.OnClickListener() {
            ...
            activityAvailableListener = new ActivityLifecycleHandler.ActivityAvailableListener() {
                public void available(@NonNull Activity activity) {
                    if (!activity.getClass().equals(PermissionsActivity.class)) {
                        ...
                        activity.overridePendingTransition(C2463R.anim.onesignal_fade_in, C2463R.anim.onesignal_fade_out);
                    }
                }
            };
